...
class PlanningGraph():
    ...
    def add_literal_level(self, level):
        ...
        # interate through each A node in previous A level
        for a_node in self.a_levels[level-1]: 
            for s_node in a_node.effnodes:
                # add previous A node's effnodes as S node in current level
                self.s_levels[level].add(s_node)
                # connect each A node with its children S node; A [level-1] => S [level]
                a_node.children.add(s_node)
                # connect each S node with its parent A node
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for action in action2.effect_rem:
            if action in action1.effect_add:
                return True

        for action in action1.effect_rem:
            if action in action2.effect_add:
                return True
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        action1 = node_a1.action
        action2 = node_a2.action
        for effect in action1.precond_pos:
            if effect in action2.precond_neg:
                return True

        for effect in action2.precond_pos:
            if effect in action1.precond_neg:
                return True

        for effect in action1.precond_neg:
            if effect in action2.precond_pos:
                return True
            
        for effect in action2.precond_neg:
            if effect in action1.precond_pos:
                return True
            ...
            for level in range (0, len(self.s_levels)):
                if first == False:
                    for state in self.s_levels[level]:
                        if g == state.symbol and state.is_pos:
                            level_sum += level
                            first = True
        return level_sum
